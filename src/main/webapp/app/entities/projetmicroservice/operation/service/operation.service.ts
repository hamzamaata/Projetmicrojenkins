...
type RestOf<T extends IOperation | NewOperation> = Omit<T, 'date' | 'heure'> & {
  date?: string | null;
  heure?: string | null;
};
...
export class OperationService {
  protected resourceUrl = this.applicationConfigService.getEndpointFor('api/operations', 'projetmicroservice');
  ...
  protected convertDateFromClient<T extends IOperation | NewOperation | PartialUpdateOperation>(operation: T): RestOf<T> {
    return {
      ...operation,
      date: operation.date?.format(DATE_FORMAT) ?? null,
      heure: operation.heure?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restOperation: RestOperation): IOperation {
    return {
      ...
      heure: restOperation.heure ? dayjs(restOperation.heure) : undefined,
    };
  }
